...
# Check if TMUX_MAIN_DISK_NAME is set or equals to __NA__
if [[ -n "$TMUX_MAIN_DISK_NAME" && "$TMUX_MAIN_DISK_NAME" != "__NA__" ]]; then
    # Get disk usage for the specific disk
    total_used=$(df -h | awk -v disk="$TMUX_MAIN_DISK_NAME" '$0 ~ disk {print $3}')
    total_size=$(df -h | awk -v disk="$TMUX_MAIN_DISK_NAME" '$0 ~ disk {print $2}')
    total_usage=$(df -h | awk -v disk="$TMUX_MAIN_DISK_NAME" '$0 ~ disk {print $5}' | sed 's/%//')
else
